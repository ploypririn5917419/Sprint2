...
public class BookingCancleController {
    ...
    public BookingCancleController(BookingCancleRepository bookingCancleRepository, BookingRepository bookingRepository,
    ...
    public Collection<Booking> booking() {
        return bookingRepository.findAll().stream().filter(this::isBooking).collect(Collectors.toList());
    }
    ...
    public Collection<TypeReason> typeReasons() {
        return typeReasonRepository.findAll().stream().collect(Collectors.toList());
    }

    @GetMapping("/bookingCancle")
    public Collection<BookingCancle> bookingCancles() {
        return bookingCancleRepository.findAll().stream().collect(Collectors.toList());
    }

    @PostMapping("/bookingCancle/{bookingId}/{bookingCancleReason}/{typeReasonName}")
    public BookingCancle newBookingCancle(@PathVariable Long bookingId, @PathVariable String bookingCancleReason,
    ...
    BookingCancle replaceBookingCancle(BookingCancle newBookingCancle, @PathVariable Long bookingId,
            @PathVariable Long bookingCancleID, @PathVariable String statusBooking) {

        return bookingCancleRepository.findById(bookingCancleID).map(booking -> {
